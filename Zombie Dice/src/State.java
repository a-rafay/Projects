...
public class State {
    ...
    //         values for the face up symbols, modifying the State in place.
    public void roll () {
	for (Die d : hand) {
	    d.roll();
	}
    ...
    //         values.  Modify the State object in place.  Note that this
    //         function assumes that there are three dice in a hand (i.e.,
    //         that the value of "hand_size" is three).
    public void roll (DieFace up1, DieFace up2, DieFace up3) {
	if (hand.size() >= 3) {
	...
	return ((current_player == Turn.user) &&
		(current_choice == Choice.stop) &&
		(comp_brains_eaten != user_brains_eaten) &&
		((comp_brains_eaten >= brains_to_win) ||
		 (user_brains_eaten >= brains_to_win)));
    }
