...
public class Eval {
	...
	static public int depth_limit = 0;
	...
	static double value_roll_hand (State s, int depth) {
		...
		// ====================================================================
		// Calculating the expected utility of the given state, given that the
		// three dice  currently in the hand will immediately be rolled by the
		// current player.  For that, I used 3 nested for loops for three dice
		// and made sure that the die face is not invalid, and then summed the
		// probability of rolling three dice times the value of outcome of the
		// rolled hand.
		// ====================================================================
					...
					if (d2 != DieFace.invalid)
					{
						for (DieFace d3 : DieFace.values())
						{
							if (d3 != DieFace.invalid)
							{
								...
								s.roll(d1, d2, d3);
								double value = Eval.value_rolled_hand(s,depth);
	...
	// value_stop -- Expected utility value of this state, 
	...
	// value_choose -- Expected utility value of each of two 
	...
	// heuristic -- Heuristic evaluation function value for the
	...
	static public double heuristic (State s) {
		...
		double value = 0.0;		// Heuristic value to be returned
		
		// Difference between brains eaten by computer and brains eaten by user
		// ==================================================
		int diff = s.comp_brains_eaten - s.user_brains_eaten;
		// ==================================================
		
		int toWin = s.brains_to_win;
		int GDIH = 0; // Green Dice in hand
		int RDIH = 0; // Red Dice in hand
		int blastsCol = s.blasts_collected;
		
		// Value to be returned is the percentage of how far the computer is from winning the game
		
		value =  (diff)*100/toWin;
		
		// The value to be returned depends upon the current state in which the computer is in at 
		// the moment. We observe that if the choice, for which we have to return the expected
		// utility of, was set originally as to "roll", then we check how many blasts has been 
		// collected at the current moment. This tells us about the goodness of the state we currently
		// are in. Since the rules shows that when number of blasts reaches 3, the turn is passed
		// on to the next player, so we have to stop roll before that. So we check if the blasts 
		// collected at the moment are less that 2, we simply return the value we calculated above.
		// But if the blasts collected are 2, and the choice is to roll, we return negative of the
		// value of the win payoff value. So that if the original choice was to roll, it should not
		// roll any further. On the other hand, if the choice is to stop, and we are in the same
		// condition, we return a positive win payoff value, so that the computer should not roll
		// any time after it has reached 2 blasts. In any other case, it can simple return the 
		// value we have calculated above.
		
		// SO THE HEURISTIC VALUE IS ENTIRELY DEPENDENT UPON THE RATIO OF DIFFERENCE BETWEEN THE SCORES
		// TO THE WIN VALUE; PROVIDED BLASTS COLLECTED ARE LESS THAN 2, AND ON BLASTS COLLECTED OTHERWISE.

		if (s.current_player == Turn.computer)
		{
			
			if (s.current_choice == Choice.roll)
			{
				if (blastsCol > 1)
				{
					return (-s.win_payoff);
			...
			else if (s.current_choice == Choice.stop)
			{
				if (blastsCol > 1)
				{
					return (s.win_payoff);
